...
// ✅ Función para obtener información de registros - MODIFICADA PARA tallerwordpress
async function obtenerInfoRegistros(db) {
  try {
    ...
    const inscritosCol = db.collection('tallerwordpress');
...
async function checkDuplicates(db, payload) {
  const col = db.collection('tallerwordpress');
  const duplicates = [];
...
// ✅ Endpoint principal para registro - MODIFICADO PARA tallerwordpress (sin tipoEstudiante ni grupo)
router.post('/registro', async (req, res) => {
  try {
    const payload = req.body || {};
    console.log('🎯 INICIANDO REGISTRO EN COLECCIÓN TALLERWORDPRESS');
    ...
    if (!infoRegistros.disponible) {
      console.log('❌ Cupo agotado para Taller de WordPress');
      return res.status(409).json({
        message: 'Cupo agotado',
        error: `Lo sentimos, no hay cupos disponibles para el Taller de WordPress. ${infoRegistros.inscritos}/${infoRegistros.cupoMaximo} usuarios registrados.`
      });
    }

    console.log('✅ Información de registros:', infoRegistros.mensaje);

    // ✅ COLECCIÓN TALLERWORDPRESS
    const col = db.collection('tallerwordpress');
    console.log('✅ Conectado a colección: tallerwordpress');
    ...
    const doc = {
      ...
      actividades: payload.actividades || ['taller-wordpress'],
      actividad: 'taller-wordpress',

      // ✅ ELIMINADO: Información del equipo (ya no aplica para participantes)

      // Metadatos del evento - ACTUALIZADO PARA TALLER DE WORDPRESS
      evento: 'Taller de WordPress',
      tipo_evento: 'taller',
      horario: 'Miércoles 12 de Noviembre de 2025, 10:00 am - 11:00 am',
    ...
    console.log('📝 Documento a guardar EN COLECCIÓN TALLERWORDPRESS:', JSON.stringify(doc, null, 2));

    // 🔹 Inserción en la colección "tallerwordpress"
    const insertRes = await col.insertOne(doc);
    const insertedId = insertRes.insertedId;

    console.log('✅✅✅ DOCUMENTO GUARDADO EN COLECCIÓN TALLERWORDPRESS CON ID:', insertedId);
    ...
    const qrPayload = {
      ...
      actividad: 'Taller de WordPress',
      evento: 'Instalación de WordPress en entorno Linux (Ubuntu)',
      horario: 'Miércoles 12 de Noviembre de 2025, 10:00 am - 11:00 am',
      lugar: 'Sala 2 de Sistemas – Sede Pance',
      ponente: 'Mag. Carlos Molina',
      emitido: nowIso
    ...
    try {
      ...
      const datosCorreo = {
        ...
        qr_image: qrDataUrl,
        evento: 'Taller de WordPress',
        actividad: 'Instalación de WordPress en entorno Linux (Ubuntu)',
        horario: 'Miércoles 12 de Noviembre de 2025, 10:00 am - 11:00 am',
        lugar: 'Sala 2 de Sistemas – Sede Pance',
        ponente: 'Mag. Carlos Molina'
      };
      ...
      // Enviar correo
      await enviarCorreoRegistro(datosCorreo, 'tallerwordpress');
      emailEnviado = true;
      console.log("✅ Correo de Taller de WordPress enviado exitosamente a:", payload.correo);
    } catch (emailError) {
    ...
    const response = {
      message: 'Inscripción al Taller de WordPress registrada correctamente',
      id: insertedId,
      qr: qrDataUrl,
      qrData: qrPayload,
      ...
      coleccion: 'tallerwordpress',
      confirmacion: 'DATOS GUARDADOS EN COLECCIÓN TALLERWORDPRESS'
    };

    console.log('✅ Respuesta exitosa:', JSON.stringify(response, null, 2));
    return res.status(201).json(response);
  } catch (err) {
    console.error('❌ Error en /tallerwordpress/registro:', err);
...
router.post('/verificar-disponibilidad', async (req, res) => {
  try {
    ...
    console.error('❌ Error en /tallerwordpress/verificar-disponibilidad:', err);
    return res.status(500).json({
      message: 'Error interno del servidor',
      error: err.message
    });
...
router.get("/estado-registros", async (req, res) => {
  try {
    ...
    console.error("❌ Error en /tallerwordpress/estado-registros:", err);
    return res.status(500).json({
      success: false,
...
// ✅ Endpoint para listar inscripciones - MODIFICADO PARA tallerwordpress (sin tipoEstudiante ni equipo)
router.get('/listar', async (req, res) => {
  try {
    const { db } = await connectMongo();
    const col = db.collection('tallerwordpress');

    console.log('📋 Listando inscripciones de la colección: tallerwordpress');
    ...
    return res.json({
      message: 'Inscripciones al Taller de WordPress encontradas',
    ...
    console.error('❌ Error en /tallerwordpress/listar:', err);
    return res.status(500).json({
      message: 'Error interno del servidor',
      error: err.message
    });
...
// ✅ Endpoint para buscar inscripción - MODIFICADO PARA tallerwordpress (sin tipoEstudiante ni equipo)
router.get('/buscar/:cedula', async (req, res) => {
  try {
    ...
    console.error('❌ Error en /tallerwordpress/buscar:', err);
    return res.status(500).json({
      message: 'Error interno del servidor',
      error: err.message
    });
