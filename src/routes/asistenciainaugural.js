...
// ✅ Función para obtener información de registros - MODIFICADA PARA asistenciainaugural
async function obtenerInfoRegistros(db) {
  try {
    ...
    const inscritosCol = db.collection('asistenciainaugural');
...
router.post('/registro', async (req, res) => {
  try {
    ...
    console.log('🎯 INICIANDO REGISTRO EN COLECCIÓN ASISTENCIAINAUGURAL');
    ...
    if (!infoRegistros.disponible) {
      console.log('❌ Cupo agotado para Asistencia Inaugural');
      return res.status(409).json({
        message: 'Cupo agotado',
        error: `Lo sentimos, no hay cupos disponibles para Asistencia Inaugural. ${infoRegistros.inscritos}/${infoRegistros.cupoMaximo} usuarios registrados.`
      });
    }

    console.log('✅ Información de registros:', infoRegistros.mensaje);

    // ✅ COLECCIÓN ASISTENCIAINAUGURAL
    const col = db.collection('asistenciainaugural');
    console.log('✅ Conectado a colección: asistenciainaugural');
    ...
    console.log('📝 Documento a guardar EN COLECCIÓN ASISTENCIAINAUGURAL:', JSON.stringify(doc, null, 2));

    // 🔹 Inserción en la colección "asistenciainaugural"
    ...
    try {
      ...
      // Enviar correo
      await enviarCorreoRegistro(datosCorreo, 'asistenciainaugural');
      emailEnviado = true;
      console.log("✅ Correo de asistencia inaugural enviado exitosamente a:", payload.correo);
    } catch (emailError) {
    ...
    const response = {
      ...
      coleccion: 'asistenciainaugural',
      confirmacion: 'DATOS GUARDADOS EN COLECCIÓN ASISTENCIAINAUGURAL'
    };

    console.log('✅ Respuesta exitosa:', JSON.stringify(response, null, 2));
    return res.status(201).json(response);
  } catch (err) {
    console.error('❌ Error en /asistenciainaugural/registro:', err);
...
router.post('/verificar-disponibilidad', async (req, res) => {
  try {
    ...
    console.error('❌ Error en /asistenciainaugural/verificar-disponibilidad:', err);
    return res.status(500).json({
      message: 'Error interno del servidor',
      error: err.message
    });
...
router.get("/estado-registros", async (req, res) => {
  try {
    ...
    console.error("❌ Error en /asistenciainaugural/estado-registros:", err);
    return res.status(500).json({
      success: false,
...
router.get('/listar', async (req, res) => {
  try {
    ...
    console.log('📋 Listando inscripciones de la colección: asistenciainaugural');
    ...
    return res.json({
      message: 'Inscripciones a la Asistencia Inaugural encontradas',
    ...
    console.error('❌ Error en /asistenciainaugural/listar:', err);
    return res.status(500).json({
      message: 'Error interno del servidor',
      error: err.message
    });
...
router.get('/buscar/:cedula', async (req, res) => {
  try {
    ...
    console.error('❌ Error en /asistenciainaugural/buscar:', err);
    return res.status(500).json({
      message: 'Error interno del servidor',
      error: err.message
    });
