...
// ✅ Función para obtener información de registros - MODIFICADA PARA construccion
async function obtenerInfoRegistros(db) {
  try {
    ...
    const inscritosCol = db.collection('construccion');
...
// ✅ Validación de campos ACTUALIZADA - Adaptada para construcción
function validatePayload(body) {
  ...
  // ✅ MODIFICADO: Validar ID para estudiantes
...
// ✅ Función para verificar duplicados en la base de datos - MODIFICADA PARA construccion
async function checkDuplicates(db, payload) {
  const col = db.collection('construccion');
  const duplicates = [];
...
// ✅ Endpoint principal para registro - MODIFICADO PARA construccion
router.post('/registro', async (req, res) => {
  try {
    const payload = req.body || {};
    console.log('🎯 INICIANDO REGISTRO EN COLECCIÓN CONSTRUCCIÓN');
    ...
    if (!infoRegistros.disponible) {
      console.log('❌ Cupo agotado para Construcción');
      return res.status(409).json({
        message: 'Cupo agotado',
        error: `Lo sentimos, no hay cupos disponibles para Construcción. ${infoRegistros.inscritos}/${infoRegistros.cupoMaximo} usuarios registrados.`
      });
    }

    console.log('✅ Información de registros:', infoRegistros.mensaje);

    // ✅ COLECCIÓN CONSTRUCCIÓN
    const col = db.collection('construccion');
    console.log('✅ Conectado a colección: construccion');
    ...
    // 🔹 Construcción del documento a guardar - ADAPTADO PARA CONSTRUCCIÓN
    const doc = {
      ...
      actividades: payload.actividades || ['construccion'],
    ...
    console.log('📝 Documento a guardar EN COLECCIÓN CONSTRUCCIÓN:', JSON.stringify(doc, null, 2));

    // 🔹 Inserción en la colección "construccion"
    const insertRes = await col.insertOne(doc);
    const insertedId = insertRes.insertedId;

    console.log('✅✅✅ DOCUMENTO GUARDADO EN COLECCIÓN CONSTRUCCIÓN CON ID:', insertedId);
    ...
    try {
      ...
      // Enviar correo
      await enviarCorreoRegistro(datosCorreo, 'construccion');
      emailEnviado = true;
      console.log("✅ Correo de construcción enviado exitosamente a:", payload.correo);
    } catch (emailError) {
    ...
    // 🔹 Respuesta exitosa
    const response = {
      message: 'Inscripción a la Actividad de Construcción registrada correctamente',
      id: insertedId,
      qr: qrDataUrl,
      qrData: qrPayload,
      ...
      coleccion: 'construccion',
      confirmacion: 'DATOS GUARDADOS EN COLECCIÓN CONSTRUCCIÓN'
    };

    console.log('✅ Respuesta exitosa:', JSON.stringify(response, null, 2));
    return res.status(201).json(response);
  } catch (err) {
    console.error('❌ Error en /construccion/registro:', err);
...
router.post('/verificar-disponibilidad', async (req, res) => {
  try {
    ...
    console.error('❌ Error en /construccion/verificar-disponibilidad:', err);
    return res.status(500).json({
      message: 'Error interno del servidor',
      error: err.message
    });
...
router.get("/estado-registros", async (req, res) => {
  try {
    ...
    console.error("❌ Error en /construccion/estado-registros:", err);
    return res.status(500).json({
      success: false,
...
// ✅ Endpoint para listar inscripciones - MODIFICADO PARA construccion
router.get('/listar', async (req, res) => {
  try {
    const { db } = await connectMongo();
    const col = db.collection('construccion');

    console.log('📋 Listando inscripciones de la colección: construccion');
    ...
    return res.json({
      message: 'Inscripciones a la Actividad de Construcción encontradas',
    ...
    console.error('❌ Error en /construccion/listar:', err);
    return res.status(500).json({
      message: 'Error interno del servidor',
      error: err.message
    });
...
// ✅ Endpoint para buscar inscripción - MODIFICADO PARA construccion
router.get('/buscar/:cedula', async (req, res) => {
  try {
    ...
    console.error('❌ Error en /construccion/buscar:', err);
    return res.status(500).json({
      message: 'Error interno del servidor',
      error: err.message
    });
