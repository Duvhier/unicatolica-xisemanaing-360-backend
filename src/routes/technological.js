    ...
    try {
        ...
        if (!infoRegistros.disponible) {
            ...
            return res.status(409).json({
                ...
                error: `Lo sentimos, no hay cupos disponibles para Technological Touch. ${infoRegistros.inscritos}/${infoRegistros.cupoMaximo} usuarios registrados.`
            });
        }
        ...
        try {
            ...
            emailEnviado = true;
            console.log("✅ Correo de Technological Touch enviado exitosamente a:", payload.correo);
        } catch (emailError) {
...
router.get("/estado-registros", async (req, res) => {
    try {
        ...
        console.error("❌ Error en /technological/estado-registros:", err);
        return res.status(500).json({
            success: false,
