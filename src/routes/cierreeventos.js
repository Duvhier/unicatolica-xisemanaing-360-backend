...
async function obtenerInfoRegistros(db) {
    try {
        ...
        const inscritosCol = db.collection('cierreeventos');
...
async function checkDuplicates(db, payload) {
    const col = db.collection('cierreeventos');
    const duplicates = [];
...
/* -------------------------------------------------------------------------- */
/* 🧾 ENDPOINT PRINCIPAL: REGISTRO DE CIERRE EVENTOS                         */
/* -------------------------------------------------------------------------- */
router.post('/registro', async (req, res) => {
    try {
        const payload = req.body || {};
        console.log('🎯 Iniciando registro de Cierre Eventos');
        ...
        try {
            ...
            await enviarCorreoRegistro(datosCorreo, 'cierreeventos');
        } catch (err) {
        ...
        res.status(201).json({
            message: 'Registro para el Cierre de Eventos realizado correctamente',
            id: insertedId,
            qr: qrDataUrl,
            cupo: {
        ...
        console.error('❌ Error en /cierreeventos/registro:', err);
...
router.get('/listar', async (req, res) => {
    try {
        ...
        res.json({
            message: 'Registros de cierre eventos encontrados',
...
router.get('/buscar/:documento', async (req, res) => {
    try {
        ...
        if (!registro) {
            return res.status(404).json({ message: 'No se encontró registro para el cierre de eventos' });
        }
